#' Interpolate values to a regular grid using linear interpolation
#'
#' These functions are both a thin wrappers around base R's `approx()` function.
#' The primary difference is that these functions output a tibble instead of 
#' a list with `x` and `y` components. The tibble / data frame output simplifies 
#' use in some work flows.
#' 
                     ...
                     )   # end grid on integer values?
{
  ...
  if(sum(is.na(.y)) >= length(.y) - 1) {
    vals <- tibble(ind = .grid, dep = NA, id = .name) %>%
      relocate(id)
    #cat('\nNAs\n')
    #print(vals$ind)
    return(vals)
    ...
    vals <- as_tibble(approx(x = .x, y = .y, xout = .grid, rule = the_rule, ...)) %>%
      rename(ind = x, dep = y) %>%
      mutate(id = .name) %>%
      relocate(id)
    return(vals)
                         ...
                         ...)
{
  ...
  if (with_zero) {
    ...
    # otherwise this crashes `approx()`
    if(sum(is.na(.y)) >= length(.y) - 1) {
      vals <- tibble(ind = ggrid, dep = NA, id = .name) %>%
        relocate(id)
      return(vals)
    }
    else {
      
      vals <- as_tibble(approx(x = .x, y = .y, xout = ggrid, rule = the_rule, ...)) %>%
        rename(ind = x, dep = y) %>%
        mutate(id = .name) %>%
        relocate(id)
      #cat('\nRegular\n')
